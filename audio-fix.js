...
(function setupPatches() {
    ...
    window.Image = function(w, h) {
        ...
        let self = {
            ...
            addEventListener: (t, cb) => { if (listeners[t]) listeners[t].push(cb); },
            removeEventListener: (t, cb) => { if (listeners[t]) listeners[t].splice(listeners[t].indexOf(cb), 1); },
            __isMockImage: true
        };
        return self;
    };
    ...
    // 4. Robust fetch patch for data: URLs
    ...
    XMLHttpRequest.prototype.send = function() {
        ...
        let self = this;
        setTimeout(() => {
            fetch(self._dataUrl).then(r => r.arrayBuffer()).then(buf => {
                self.readyState = 4;
                self.status = 200;
                self.response = buf;
    ...
    // 6. Web Audio API patch - intercept audio context createMediaElementAudioSource
    if (window.AudioContext || window.webkitAudioContext) {
        let AudioCtx = window.AudioContext || window.webkitAudioContext;
        let _origCreateMedia = AudioCtx.prototype.createMediaElementAudioSource;
        if (_origCreateMedia) {
            AudioCtx.prototype.createMediaElementAudioSource = function(elem) {
                console.log('[PATCHES] Audio element source:', elem.src);
                return _origCreateMedia.call(this, elem);
            };
        }
    }
